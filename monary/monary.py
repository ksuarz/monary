...
def _load_coldata_from_mps(coldata, params):
    """For each given param, load a column into the coldata.

       :param coldata: The column data into which the columns will be loaded.
       :param params: A list of MonaryParams to be used as columns.
    """
...
class Monary(object):
    ...
    def insert(self, db, coll, params, write_concern=None):
        ...
        try:
            coldata = cmonary.monary_alloc_column_data(len(params),
                                                       len(params[0]))
            _load_coldata_from_mps(coldata, params)
    ...
    def update(self, db, coll, sel_params, doc_params, upsert=False,
        ...
        """Performs a bulk update/upsert based on data from arrays.
           ...
           :rtype: int
        """
        if len(sel_params) == 0 or len(doc_params) == 0:
            raise ValueError("cannot do an empty update")

        validate_bson_fields(list(map(lambda p: p.field, sel_params)), True)
        validate_bson_fields(list(map(lambda p: p.field, doc_params)), True)

        sel_params = sorted(
            sel_params, key=lambda p: p.field if p.field != "_id" else chr(0))

        doc_params = sorted(
            doc_params, key=lambda p: p.field if p.field != "_id" else chr(0))

        if len(set(map(len, sel_params + doc_params))) > 1:
            raise ValueError("all data arrays must be of the same length")

        collection = None
        try:
            num_rows = len(doc_params[0])

            sel_coldata = cmonary.monary_alloc_column_data(len(sel_params),
                                                           num_rows)
            _load_coldata_from_mps(sel_coldata, sel_params)

            doc_coldata = cmonary.monary_alloc_column_data(len(doc_params),
                                                           num_rows)
            _load_coldata_from_mps(doc_coldata, doc_params)
            ...
            if upsert:
                ...
                id_type = "id"
                if sel_params[0].field == "_id":
                    id_type = sel_params[0].mtype
                cmonary_type, cmonary_type_arg, numpy_type = \
